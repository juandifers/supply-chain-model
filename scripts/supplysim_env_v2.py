...
class SupplySimEnvV2:
    ...
    def reset(
        ...
        (self.firms, self.products, self.prod_graph,
         self.firm2prods, self.prod2firms, self.inputs2supplier) = sd.generate_static_graphs(
            ...
            min_num_suppliers=self.min_num_suppliers,
            max_num_suppliers=self.max_num_suppliers,
    ...
    def step(self, action=None, debug=False, log_kpis=None):
        ...
        for key in list(self.curr_orders.keys()):
            surviving = []
            for order in self.curr_orders[key]:
                buyer, amt = order[0], order[1]
                t_placed = order[2] if len(order) > 2 else 0
                if (self.t - t_placed) <= self.max_order_age:
        ...
        if self.demand_schedule is not None:
            ...
            for p in self.consumer_prods:
                for f in self.prod2firms[p]:
                    ...
                    self.curr_orders[(f, p)].append(("consumer", demand_val, self.t))
                    demand_added_units += demand_val
                    demand_added_orders += 1
        if self.t >= self.kpi_start_step:
        ...
        kpis = {
            ...
            "lost_sales_units": float(np.round(lost_sales_units, self.num_decimals)),
        }
        ...
        if "reroute" in action:
            ...
            for buyer, product, new_supplier in reroutes:
                ...
                if old_supplier is not None and old_supplier != new_supplier:
                    ...
                    old_orders = self.curr_orders.get(old_key, [])
                    transferred = []
                    remaining = []
                    ...
                    self.curr_orders[old_key] = remaining
